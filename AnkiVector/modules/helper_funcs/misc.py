...
def paginate_modules(page_n: int, module_dict: Dict, prefix, chat=None) -> List:
    ...
    if len(pairs) > 9:
        pairs = pairs[modulo_page * 9:9 * (modulo_page + 1)] + [
            (EqInlineKeyboardButton("ᐊBack", callback_data="{}_prev({})".format(prefix, modulo_page)),
                EqInlineKeyboardButton("🔙 Back", callback_data="helpmenu_"),
             EqInlineKeyboardButton("Nextᐅ", callback_data="{}_next({})".format(prefix, modulo_page)))]

    else:
        pairs += [[EqInlineKeyboardButton("🔙 Back", callback_data="helpmenu_")]]
