...
async def useradd(_, message: Message):
    if not message.reply_to_message:
        ...
        if added:
            ...
            os.system(f"kill -9 {os.getpid()} && python3 -m AnkiVector")
        else:
...
async def userdel(_, message: Message):
    if not message.reply_to_message:
        ...
        if removed:
            await message.reply_text(
                f"Removed **{user.mention}** from SheLa's Sudo."
            )
            return os.system(f"kill -9 {os.getpid()} && python3 -m AnkiVector")
    ...
    if user_id not in SUDOERS:
        return await message.reply_text(
            f"Not a part of SheLa's Sudo."
        )
    removed = await remove_sudo(user_id)
    if removed:
        await message.reply_text(
            f"Removed **{mention}** from SheLa's Sudo."
        )
        return os.system(f"kill -9 {os.getpid()} && python3 -m AnkiVector")
    await message.reply_text(f"Something wrong happened.")
    ...
    for x in served_chats:
        try:
            await app.send_message(
                x,
                f"SheLa has just restarted herself. Sorry for the issues.\n\nStart playing after 10-15 seconds again.",
            )
            await remove_active_chat(x)
        except Exception:
            pass
    x = await message.reply_text(f"Restarting SheLa")
    os.system(f"kill -9 {os.getpid()} && python3 -m AnkiVector")
...
async def update(_, message):
    ...
    if str(m[0]) != "A":
        ...
        return os.system(f"kill -9 {os.getpid()} && python3 -m -m AnkiVector")
    else:
